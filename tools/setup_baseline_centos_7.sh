...
if ! grep -ie '.*CentOS.* 7.[0-9]\+.[0-9]\+ .*' /etc/system-release >/dev/null; then
    echo -e '\nOnly for CentOS 7.x .'
...
echo
echo -e '                 Installation of \e[92mBaseline 5.x\e[0m'
echo -e '                  \e[4mbased on \e[92mCentOS 7.x x86_64\e[0m'
echo
...
echo ' default configuration to customize to baseline 5.x.'
echo -en '\e[96m'
printf '%.1s' '='{0..61}
echo -e '\e[0m'
echo
...
function hasVBoxGuestAdditions() {
    systemctl status vboxadd >/dev/null 2>&1
    return $?
}
...
if isVirtualBox; then
    ...
    if hasVBoxGuestAdditions; then
        ...
        echo
        systemctl list-units --type service | grep -i 'vbox\|virtualbox'
        echo
...
if pask 'Disable IPv6'; then
    # disable ipv6 in current runtime os.
    ...
    # disable ipv6 always even reboot.
    conf_file='/etc/sysctl.d/disableipv6.conf'
    if echo "$conf_all = 1" > $conf_file; then
        echo -e "\nConfirm:"
        seg '/sbin/ip -6 addr'
        ...
        seg '/etc/sysctl.d/*.conf'
        grep -i ipv6 /etc/sysctl.d/*.conf || echo 'No ipv6 entry.'
    else
...
pdate
if pask 'Create pre-defined groups: program, staff, maintenance'; then
    createGroup program     1001
    createGroup staff       2001
...
if pask 'Install yum plugins'; then
    yum -y install yum-plugin-fastestmirror yum-plugin-priorities
    ret=$?
    echo
...
if pask 'Set system default language to en_US.UTF-8'; then
    localectl set-locale LANG=en_US.UTF-8 || errmsg 'Error: failed!'
    echo
    localectl
fi
...
if pask 'Create pre-defined user'; then
    ...
    echo
    if pask "Create name=$username uid=$uid groups=staff,wheel"; then
        useradd -m -g staff -G wheel -s `which bash` -u $uid -p `openssl passwd -1 $passwd` $username
        if [ $? -eq 0 ]; then
            echo
            id $username
            echo 'Install OK.'
...
echo 'Maybe the system need restart.'
echo
